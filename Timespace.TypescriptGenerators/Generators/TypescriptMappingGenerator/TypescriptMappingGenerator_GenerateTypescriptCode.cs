...
public partial class TypescriptMappingGenerator
{
    ...
    private Dictionary<string, ApiType> GetChildrenFromRootTypeName(string rootTypeName, Dictionary<string, ApiType> apiTypes, Dictionary<string, ApiType> sharedTypes)
    {
            ...
            .Select(x =>
                {
                    if (x.Value is ApiTypeClass)
                    {
                        return new KeyValuePair<string, SharedType>(
                            x.Key,
                            new SharedType(x.Value.TypeName, $"{x.Value.TypeName}ResponseSchema", $"{x.Value.TypeName}ResponseSchemaStringified", $"{x.Value.TypeName}RequestSchema", $"{x.Value.TypeName}RequestSchemaStringified", $"$api/shared/{x.Value.TypeName.ToCamelCase()}", false));
                    }
                    else
                    {
                        return new KeyValuePair<string, SharedType>(
                            x.Key,
                            new SharedType(x.Value.TypeName, $"z.nativeEnum({x.Value.TypeName})", $"z.nativeEnum({x.Value.TypeName})", $"z.number()", "z.number()", $"$api/enums/{x.Value.TypeName.ToCamelCase()}", true));
                    }
                })
            .ToDictionary();
    }
    ...
    private TypescriptSourceFragment GenerateTypescriptType(ApiType type, Dictionary<string, SharedType> sharedTypes, ApiTypeCategory typeCategory, bool generateStringified = false)
    {
    ...
    private TypescriptSourceFragment GenerateTypescriptFromEnumType(ApiTypeEnum apiTypeEnum, Dictionary<string, SharedType> sharedTypes)
    {
        ...
        {
            importables.Add(new(ImportType.Type, sharedType, ImportFlags.Type));
        }
        else
        {
    ...
    private TypescriptSourceFragment GenerateTypescriptFromClassType(ApiTypeClass apiTypeClass, Dictionary<string, SharedType> sharedTypes, ApiTypeCategory typeCategory, bool generateStringified)
    {
        var importables = new HashSet<TypescriptImportable>();
        var sourceBuilder = new StringBuilder();
        importables.Add(new(ImportType.Zod, null, null));
        if(typeCategory == ApiTypeCategory.Shared)
            generateStringified = true;
        
        ImportFlags importFlags = GetImportFlags(typeCategory, generateStringified);
    ...
    private string GenerateTypescriptInterface(ApiTypeClass apiTypeClass, Dictionary<string, SharedType> sharedTypes, HashSet<TypescriptImportable> importables, ImportFlags importFlags)
    {
        var sourceBuilder = new StringBuilder();
        
        if (sharedTypes.TryGetValue(apiTypeClass.FullyQualifiedTypeName, out var sharedType))
        {
            importables.Add(new(ImportType.Type, sharedType, sharedType.IsEnum ? ImportFlags.Type : importFlags));
        }
        else
        {
            ...
            foreach (var property in apiTypeClass.Properties)
            {
                ...
                {
                    importables.Add(new(ImportType.Type, type, type.IsEnum ? ImportFlags.Type : importFlags));
                }

                if (property.FullyQualifiedTypeName.Replace("?", "") is "global::NodaTime.Instant" or "global::NodaTime.LocalDate")
                    importables.Add(new(ImportType.Dayjs, null, null));
                
                string tsType = GetMapping(property.FullyQualifiedTypeName)?.TypescriptType ?? type?.TypeName ?? property.TypeName;
    ...
    private string GenerateZodToType(ApiTypeClass apiTypeClass, Dictionary<string, SharedType> sharedTypes, HashSet<TypescriptImportable> importables, ApiTypeCategory typeCategory, ImportFlags importFlags, bool generateStringified = false)
    {
        var sourceBuilder = new StringBuilder();
        ...
        {
            sourceBuilder.AppendLine($"{(typeCategory is ApiTypeCategory.Shared ? "export " : "")}const {apiTypeClass.TypeName}ResponseSchema{(generateStringified ? "Stringified" : "")} = z.object({{");
            foreach (var property in apiTypeClass.Properties)
            {
                if(sharedTypes.TryGetValue(property.FullyQualifiedTypeName, out var type))
                    importables.Add(new(ImportType.Type, type, type.IsEnum ? ImportFlags.Type : importFlags));
                ...
                string zodMapping = GetMapping(property.FullyQualifiedTypeName)?.ZodToMapping ?? 
    ...
    private string GenerateZodFromType(ApiTypeClass apiTypeClass, Dictionary<string, SharedType> sharedTypes, HashSet<TypescriptImportable> importables, ApiTypeCategory typeCategory, ImportFlags importFlags, bool generateStringified = false)
    {
        var sourceBuilder = new StringBuilder();
        ...
        {
            sourceBuilder.AppendLine($"{(typeCategory is ApiTypeCategory.Shared ? "export " : "")}const {apiTypeClass.TypeName}RequestSchema{(generateStringified ? "Stringified" : "")} = z.object({{");
            foreach (var property in apiTypeClass.Properties)
            {
                if(sharedTypes.TryGetValue(property.FullyQualifiedTypeName, out var type))
                    importables.Add(new(ImportType.Type, type, type.IsEnum ? ImportFlags.Type : importFlags));
                ...
                string zodMapping = GetMapping(property.FullyQualifiedTypeName)?.ZodFromMapping ?? 
    ...
    private string GetZodPropertyType(ApiClassProperty property, string zodMapping, bool isEnum = false, bool generateStringified = false)
    {
        ...
        bool isDefaultMappable = GetMapping(property.FullyQualifiedTypeName) == null;
        bool shouldStringify = generateStringified && (!isDefaultMappable && !nonStringifyableTypes.Contains(property.FullyQualifiedTypeName.Replace("?", "")) || isEnum);
        
        return property.CollectionInfo.CollectionType switch
        {
            CollectionType.None => zodMapping + (property.IsNullable ? nullableExtension : "") + (shouldStringify ? $".transform((v: any) => v{(property.IsNullable ? "?" : "")}.toString())" : ""),
            CollectionType.List => $"z.array({zodMapping}{(property.IsNullable ? nullableExtension : "")}){(property.CollectionInfo.IsNullable ? ".nullish()" : "")}",
            CollectionType.Dictionary => $"z.record({zodMapping}{(property.IsNullable ? nullableExtension : "")}){(property.CollectionInfo.IsNullable ? ".nullish()" : "")}",
            _ => throw new Exception($"Unknown collection type {property.CollectionInfo.CollectionType}")
        };
    }

    private DefaultTypeMapping? GetMapping(string typeName) => Constants.DefaultTypeMappings.FirstOrDefault(x => x.Key == typeName.Replace("?", "")).Value;
    private ImportFlags GetImportFlags(ApiTypeCategory typeCategory, bool generateStringified) => typeCategory switch
    {
        ApiTypeCategory.Request => generateStringified ? ImportFlags.Type | ImportFlags.ResponseStringified : ImportFlags.Type | ImportFlags.Response,
        ApiTypeCategory.Response => generateStringified ? ImportFlags.Type  | ImportFlags.RequestStringified : ImportFlags.Type | ImportFlags.Request,
        ApiTypeCategory.Shared => generateStringified ? ImportFlags.Type | ImportFlags.Response | ImportFlags.ResponseStringified | ImportFlags.Request | ImportFlags.RequestStringified : ImportFlags.Type | ImportFlags.Response | ImportFlags.Request,
    ...
    {
        List<EndpointParameter> pathParameters = endpoint.Parameters.Where(x => x.Source == ParameterSource.Path).ToList();
        Dictionary<string, ApiType> requestTypes = endpoint.RequestTypes;
        string routeUrl = endpoint.RouteUrl;
        
        var sourceBuilder = new StringBuilder();
        
        ApiType? queryParamTypeName = requestTypes.FirstOrDefault(x => x.Key == endpoint.QueryTypeName).Value;
        ApiType? pathParamTypeName = requestTypes.FirstOrDefault(x => x.Key == endpoint.PathTypeName).Value;
        ...
        sourceBuilder.AppendLine($"export const {endpoint.ActionName.ToCamelCase()}Url = ({string.Join(", ", methodParams.Select(x => $"{x.Key}: {x.Value}"))}): string => {{");
        ...
        return new TypescriptSourceFragment(new(), sourceBuilder.ToString());
    }

    private TypescriptSourceFragment GenerateFetchMethod(ApiEndpoint endpoint)
    {
        var importables = new HashSet<TypescriptImportable>();
        importables.Add(new(ImportType.FetchType, null, null));
        importables.Add(new(ImportType.ProblemDetails, null, null));
        importables.Add(new(ImportType.BaseUrl, null, null));
        if(endpoint.FormData)
            importables.Add(new(ImportType.Formdata, null, null));
        ...
        var fetchFunction = endpoint.HttpMethod switch
        {
            ...
            "POST" => GeneratePostLike(endpoint.HttpMethod, endpointTypes[endpoint.BodyTypeName ?? throw new Exception("POST method should have body type")].TypeName, endpoint.FormData),
            "PUT" => GeneratePostLike(endpoint.HttpMethod, endpointTypes[endpoint.BodyTypeName ?? throw new Exception("PUT method should have body type")].TypeName, endpoint.FormData),
            "PATCH" => GeneratePostLike(endpoint.HttpMethod, endpointTypes[endpoint.BodyTypeName ?? throw new Exception("PATCH method should have body type")].TypeName, endpoint.FormData),
