...
internal partial class TypescriptMappingGenerator
{
	private ApiEndpoint? TransformEndpointDescription(EndpointDescription description)
	{
		...
		{
			_logger.LogError("API endpoint with route url {RouteUrl} does not have a parent controller type that could be found",
		...
		{
			_logger.LogError("API endpoint with route url {RouteUrl} does not have a valid action symbol that could be found.",
				description.RelativePath);
			return null;
		}
	...
	private void GenerateApiTypeFromEndpointParameters(List<EndpointParameter> endpointParameters, string typeNamePrefix, string typeName,
		Dictionary<string, ApiType> generatableTypes)
	{
	...
	private void GenerateApiTypeFromTypeSymbol(ITypeSymbol typeSymbol, Dictionary<string, ApiType> generatableTypes)
	{
		...
		else
		{
			foreach (var propertySymbol in propertySymbols)
			{
				...
				{
					_logger.LogError(
						"Type {TypeName} has a property of type {PropertyTypeName} which is the same as the parent type. This is not supported.",
						typeSymbol.Name, resolvedPropertySymbolType.Name);
