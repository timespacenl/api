...
public static class TypescriptSourceFragmentExtensions
{
	public static TypescriptSourceFile GetSourceFile(this List<TypescriptSourceFragment> fragments, string filePath, string fileName)
	{
		...
		if (imports.Any(x => x.ImportableType is not null))
		{
			...
			{
				var fromImport = importableType.ImportFlags is not null && importableType.ImportFlags.Value.HasFlag(ImportFlags.Response)
					? $", {importableType.ImportableType!.ResponseZodType}"
					: "";
				var toImport = importableType.ImportFlags is not null && importableType.ImportFlags.Value.HasFlag(ImportFlags.Request)
					? $", {importableType.ImportableType!.RequestZodType}"
					: "";
				var fromImportStringified =
					importableType.ImportFlags is not null && importableType.ImportFlags.Value.HasFlag(ImportFlags.ResponseStringified)
						? $", {importableType.ImportableType!.ResponseZodTypeStringified}"
						: "";
				var toImportStringified =
					importableType.ImportFlags is not null && importableType.ImportFlags.Value.HasFlag(ImportFlags.RequestStringified)
